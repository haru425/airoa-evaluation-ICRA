...
from openpi.policies import calvin_actions
...
class CalvinInputs(transforms.DataTransformFn):
    """Converts CALVIN observations into the common model input format."""
...
class CalvinOutputs(transforms.DataTransformFn):
    """Converts model outputs back into the 7D CALVIN action space."""

    def __call__(self, data: dict) -> dict:
        return {"actions": calvin_actions.to_calvin_action_chunk(data["actions"])}
