    ...
    def setup(self):
        ...
        if config := self.lora_config:
            ...
            if self.num_adapters == 1:
                ...
                shape_a[config.axes[1]] = config.rank
                shape_b[config.axes[0]] = config.rank
                self.w_a = self.param("lora_a", config.init_fn, (self.num_adapters, *tuple(shape_a)))
                self.w_b = self.param("lora_b", config.init_fn, (self.num_adapters, *tuple(shape_b)))

    @nn.compact
    def __call__(self, eqn: str, x, adapter_id):
        assert adapter_id >= 0
        dtype = x.dtype  # original dtype, could be half-precision
        result = jnp.einsum(eqn, x, self.w.astype(dtype))

        if config := self.lora_config:
            w_a = self.w_a if self.num_adapters == 1 else self.w_a[adapter_id]
            w_b = self.w_b if self.num_adapters == 1 else self.w_b[adapter_id]
            eqn_a, eqn_b = self._make_lora_eqns(eqn)
            lora = jnp.einsum(eqn_a, x, w_a.astype(dtype))
            lora = jnp.einsum(eqn_b, lora, w_b.astype(dtype))
            ...
            if self.num_adapters == 1:
                ...
                self.w_gating_lora = (
                    self.param(
                        ...
                        (self.num_adapters, 2, self.features, self.lora_config.rank),
                    ),
                    self.param(
                        "gating_einsum_lora_b",
                        self.lora_config.init_fn,
                        (self.num_adapters, 2, self.lora_config.rank, self.hidden_dim),
                    ),
                )
                self.w_linear_lora = (
                    self.param(
                        "linear_lora_a",
                        self.lora_config.init_fn,
                        (self.num_adapters, self.hidden_dim, self.lora_config.rank),
                    ),
                    self.param(
                        "linear_lora_b",
                        self.lora_config.init_fn,
                        (self.num_adapters, self.lora_config.rank, self.features),
                    ),
                )

    @nn.compact
    def __call__(self, x, adapter_id):
        assert adapter_id >= 0
        dtype = x.dtype  # original dtype, could be half-precision
        if self.w_gating_lora is None:
            ...
            w_gating_ff = (self.w_gating_lora[0][0], self.w_gating_lora[1][0])
            w_gating_ff1 = (self.w_gating_lora[0][1], self.w_gating_lora[1][1])
        else:
            w_gating_ff = (self.w_gating_lora[0][adapter_id][0], self.w_gating_lora[1][adapter_id][0])
            w_gating_ff1 = (self.w_gating_lora[0][adapter_id][1], self.w_gating_lora[1][adapter_id][1])
        ...
        if self.num_adapters > 1:
            w_liner = (self.w_linear_lora[0][adapter_id], self.w_linear_lora[1][adapter_id])
        else:
            w_liner = self.w_linear_lora
            
        outputs = self._dot(activations, self.w_linear, w_liner)
